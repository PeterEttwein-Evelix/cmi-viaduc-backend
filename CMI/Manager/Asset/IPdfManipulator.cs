...
namespace CMI.Manager.Asset
{
    public interface IPdfManipulator
    {
        ...
        /// <param name="splitEvenWhenPdfContainsTextLayer">if set to true, pdf with a textlayer are not splitted</param>
        /// <returns></returns>
        List<AssetConversionFile> ConvertToConversionFiles(List<FileInfo> files, bool splitEvenWhenPdfContainsTextLayer);
    ...
    public class PdfManipulator : IPdfManipulator
    {
        ...
        public List<AssetExtractionFile> ConvertToTextExtractionFiles(List<RepositoryFile> repositoryFiles, string path)
        {
            ...
            try
            {
                foreach (var repositoryFile in repositoryFiles)
                {
                    ...
                    // Skip files that were not exported
                    if (!repositoryFile.Exported)
                    {
                        Log.Information("Skipping {repositoryFile} as it was not downloaded from the repository", repositoryFile.PhysicalName);
                        continue;
                    }
                    ...
                    {
                        // Split if necessary
                        if (NeedsSplitting(diskFile, false))
                        {
                            isSplitted = true;
        ...
        public List<AssetConversionFile> ConvertToConversionFiles(List<FileInfo> files, bool splitEvenWhenPdfContainsTextLayer)
        {
            ...
            try
            {
                foreach (var file in files)
                {
                    ...
                    {
                        // Split if necessary
                        if (NeedsSplitting(file, splitEvenWhenPdfContainsTextLayer))
                        {
                            isSplitted = true;
        ...
        private static bool HasText(Document document)
        {
            try
            {
